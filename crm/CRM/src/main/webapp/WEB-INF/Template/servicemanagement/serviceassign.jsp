...
<html>
  <head>
    ...
    <script type="text/javascript">
        ...
        function assign(id)
        {
        	var value = $('#due_to'+id+' option:selected').val();
        	window.location.href='service/assign_Service?id='+id+'&due_to='+value;
        }

    </script>
</head>
<body>
    <div class="alert alert-info">当前位置<b class="tip"></b>客户服务管理<b class="tip"></b>服务分配</div>
    ...
    <table class="table table-striped table-bordered table-condensed" id="list">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                <c:if test='${list.svc_status=="新创建" }'>
                <select style="width:100px;" id="due_to${list.svc_id }">
                <c:forEach items="${users }" var="user">
                 
                   <option value="${user.name }">${user.name }</option>
                </c:forEach>
                </select>
                <input style="width:50px;" type="button" onclick="assign(${list.svc_id})" value="分配" class="btn btn-inverse"/>
                </c:if>
                 <c:if test='${list.svc_status!="新创建" }'>
                 	已分配
                 </c:if>
                </td>
                <td><button  onclick="window.location.href='service/delete_Service?id=${list.svc_id}'">删除</button ></td>
