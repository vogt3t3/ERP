    ...
    <table width="80%"  class="table table-striped table-bordered table-condensed list">
           <tr>
            ...
            <td>${loss.lst_status }</td>
             <td><button onclick="window.location.href='client/open_Lst?lstId=${loss.lst_id }'">暂缓流失</button><button onclick="window.location.href='client/open_sureLst?lstId=${loss.lst_id }'">确认流失</button></td>
         	</tr>
         	</c:forEach>
